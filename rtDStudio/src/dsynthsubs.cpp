...
void DSynthSubS::Set(const Config& config)
{
    base_config_ = config;
    ...
    // noise, shared
    // already inited, no setup needed
	...
	for (uint8_t i = 0; i < voices_; i++)
	{
        ...
        osc0_[i].SetFreq(440.0f); // default

		osc1_[i].SetWaveform(waveform1_);
		osc1_[i].SetAmp(0.5f); // default
        osc1_[i].SetFreq(440.0f + detune_); // default
...
void DSynthSubS::Process(float *out_l, float *out_r)
{
    *out_l = Process();
    *out_r = *out_l;
}
...
void DSynthSubS::MidiIn(uint8_t midi_status, uint8_t midi_data0, uint8_t midi_data1 = 0)
{
    // OMNI - ignore channel
...
void DSynthSubS::NoteOn(uint8_t midi_note, uint8_t midi_velocity)
{
...
//    note_freq_[osc_next_] = daisysp::mtof(note_midi_[osc_next_]);
    ...
    osc0_[osc_next_].SetFreq(note_freq_[osc_next_]);
    osc1_[osc_next_].SetFreq(note_freq_[osc_next_] + detune_);
...
void DSynthSubS::NoteOff(uint8_t midi_note)
{
	for (uint8_t i = 0; i < voices_; i++)
	{
...
void DSynthSubS::SetWaveform(Waveform waveform0, Waveform waveform1)
{
...
void DSynthSubS::SetTuning(float detune)
{
    detune_ = detune;
}
...
void DSynthSubS::SetLevel(float osc0_level, float osc1_level, float noise_level)
{
...
void DSynthSubS::SetFilter(FilterType filter_type, float filter_cutoff, float filter_res)
{
...
void DSynthSubS::SetFilterFreq(float filter_cutoff)
{
    filter_cutoff_ = filter_cutoff;
...
void DSynthSubS::SetFilterRes(float filter_res)
{
    filter_res_ = filter_res;
...
void DSynthSubS::SetEGLevel(Target target, float level)
{
    switch (target)
    {
        case PITCH:
...
void DSynthSubS::SetEG(Target target, float eg_attack, float eg_decay, float eg_sustain, float eg_release)
{
    switch (target)
    {
        case PITCH:
...
void DSynthSubS::SetLFO(Waveform lfo_waveform, float lfo_freq, float lfo_amp, float lfo_p_level, float lfo_f_level, float lfo_a_level)
{
...
void DSynthSubS::SetPortamento(float portamento)
{
...
void DSynthSubS::SetDelay(float delay_delay, float delay_feedback)
{
...
*/
void DSynthSubS::ChangeParam(DSynth::Param param, float value)
{
    switch (param)
    {
        ...
        case DSynth::DSYNTH_PARAM_DETUNE:
            SetTuning(base_config_.detune * value);
            break;
...
void DSynthSubS::SetLevel(float level)
{
